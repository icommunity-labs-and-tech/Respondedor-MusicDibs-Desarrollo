...
export default function InboxPage() {
  ...
  async function handleCheckEmails() {
    ...
    try {
      ...
      if (res.ok) {
        setCheckResult({
          message: `${data.newEmails || 0} Nuevos emails recibidos`,
          type: "success",
        });
        loadEmails();
      } else {
        ...
        <div className="p-4 pb-3 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              title="Comprobar nuevos emails"
              className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-semibold text-on-surface bg-surface-container-lowest hover:bg-surface-container rounded-lg transition-colors disabled:opacity-60 border border-outline-variant/20"
            >
              {checkingEmails ? (
                <>
